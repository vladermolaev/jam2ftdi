...
int jam_jtag_io(int tms, int tdi, int read_tdo)
{
	...
	if (specified_com_port)
	{
		...
		if (read_tdo)
		{
			...
			{
				result = _read(com_port, &ch_data, 1);
			}
			if (result == 1)
			{
				tdo = ch_data & 0x01;
...
#if PORT == WINDOWS || PORT == DOS || PORT == FTDI
	...
	for (arg = 1; arg < argc; arg++)
	{
...
#endif
		{
			...
			case 'P':				/* set LPT port address */
				...
				if (sscanf_s(&argv[arg][2], "%d", &lpt_port) != 1) error = TRUE;
				if ((lpt_port < 1) || (lpt_port > 3)) error = TRUE;
				if (error)
				{
					if (sscanf_s(&argv[arg][2], "%x", &lpt_port) == 1)
					{
						if ((lpt_port == 0x278) ||
							(lpt_port == 0x27c) ||
							(lpt_port == 0x378) ||
			...
			case 'M':				/* set memory size */
				if (sscanf_s(&argv[arg][2], "%ld", &workspace_size) != 1)
					error = TRUE;
...
)
{
		...
		== ERROR_SUCCESS)
	{
		...
		if (RegQueryValueEx(
			...
			== ERROR_SUCCESS)
		{
			if ((type == REG_SZ) && (_stricmp(buffer, hardware_key) == 0))
			{
				size = 1023;
		...
		do
		{
			...
			if (result == ERROR_SUCCESS)
			{
				dd_path[length] = '\\';
				dd_path[length + 1] = '\0';
				strcpy_s(&dd_path[length + 1], dd_path_size-length-1, buffer);

				search_dyn_data(dd_path, dd_path_size, hardware_key, lpt);
		...
		== ERROR_SUCCESS)
	{
			...
			== ERROR_SUCCESS)
		{
				...
				(buffer[4] == '\0'))
			{
				...
				search_dyn_data(dd_path, dd_path_size, &lm_path[5], (buffer[3] - '1'));
			}
		}
		...
		do
		{
			...
			if (result == ERROR_SUCCESS)
			{
				lm_path[length] = '\\';
				lm_path[length + 1] = '\0';
				strcpy_s(&lm_path[length + 1], lm_path_size-length-1, buffer);

				search_local_machine(lm_path, lm_path_size, dd_path, dd_path_size);
...
void get_lpt_addresses_from_registry()
{
	...
	search_local_machine(lm_path, sizeof(lm_path), dd_path, sizeof(dd_path));
}
#endif
#endif
...
{
#if PORT == FTDI
  int status = CB_BB_OPEN_ERROR_OPEN_PORT;
  for (int d = 0; d < 100 && status != CB_OK; ++d) status = FTDI_Open(d);
  if (status == CB_OK) return;
  fprintf(stderr, "cannot open FTDI device\n");
#else
	if (specified_com_port)
	{
		auto result = _sopen_s(&com_port, serial_port_name, O_RDWR, _SH_DENYRW, _S_IREAD | _S_IWRITE);
		if (result != 0 || com_port == -1)
		{
		...
		else
		{
			...
			data = 0x7e;
			_write(com_port, &data, 1);

			for (i = 0; (i < 100) && (result != 1); ++i)
			{
				result = _read(com_port, &data, 1);
			}

			if (result == 1)
			{
				data = 0x70; _write(com_port, &data, 1); /* TDO echo off */
				data = 0x72; _write(com_port, &data, 1); /* auto LEDs off */
				data = 0x74; _write(com_port, &data, 1); /* ERROR LED off */
				data = 0x76; _write(com_port, &data, 1); /* DONE LED off */
				data = 0x60; _write(com_port, &data, 1); /* signals low */
			}
			else
			{
